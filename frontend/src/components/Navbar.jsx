...
export default function Navbar() {
  ...
  // Added 'font-sans' here to apply Arial/Helvetica to the whole link
    ...
    <>
      ...
      <header className="fixed top-0 left-0 w-full h-[90px] bg-white flex justify-between items-center px-6 border-b border-[#D9D9D9] z-[1000]">
        ...
        <div className="flex items-center gap-4 md:pr-12">
          ...
          {/* User Profile */}
          <div className="flex items-center gap-1.5 cursor-pointer p-1 rounded-lg hover:bg-gray-100 transition-colors">
            
            <MdKeyboardArrowDown className="text-xl text-[#9F29B5]" />
          </div>
        </div>
      </header>
      ...
      `}>
        ...
        <div className="pt-4">
          ...
          <Link to="/budget" className={getLinkStyle("/budget")}>
            <MdAccountBalanceWallet className="text-xl mr-4" />
            <span className="md:max-lg:hidden">Budget</span>
          </Link>

          <Link to="/expenses" className={getLinkStyle("/expenses")}>
            <MdReceipt className="text-xl mr-4" />
            <span className="md:max-lg:hidden">Expenses</span>
          </Link>

          <Link to="/reports" className={getLinkStyle("/reports")}>
            <MdAssessment className="text-xl mr-4" />
            <span className="md:max-lg:hidden">Reports</span>
          </Link>
        </div>

        <div className="pb-8">
          <Link to="/login" className="flex items-center px-6 py-3 mx-4 text-[#333] font-medium border-t border-gray-100 font-['Arial',_Helvetica,_sans-serif] hover:text-[#9F29B5]">
